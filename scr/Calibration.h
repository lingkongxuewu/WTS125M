...
/*-------------------------结构体数据类型定义--------------------------*/
	struct temp_calib{
		...
		int16s adcarr[TEMP_CAL_NUM];                //ADC缓存数组
...
//	int8u GetTempYCalCnt(void);                    //获取Y轴温度补偿点的个数
	...
	int16u GetTempXCalAdcVal(int8u pos);           //获取X轴温度补偿中ADC数组中的某个AD值
	...
	void SetTempXCalAdcVal(int16s val, int8u pos);  //设置X轴温度补偿中ADC数组中的某个AD值
...
//CALIBR_EXT struct temp_calib xdata Temp_McuTcal;	 //hxl mcu温度校准

CALIBR_EXT struct ang_calib  xdata Ang_Xcal;
CALIBR_EXT struct ang_calib  xdata Ang_Ycal;
CALIBR_EXT int8s xdata SetMcuTincelsius;
CALIBR_EXT int16u xdata McuTAdcValWhileSet;

//CALIBR_EXT  int16u xdata temp_buf[TEMP_CAL_NUM]; 	